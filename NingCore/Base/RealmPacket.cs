...
{
    public enum REALM_OPCODE : UInt16
    {
        NONE = 0,
    ...
    class RealmPacket : Packet
    {
        public RealmPacket(byte[] pmBytes)
        {
            headPart = new PacketPart(pmBytes.Take(6).ToArray());
            bodyPart = new PacketPart(pmBytes.Skip(6).ToArray());
            opcode = (REALM_OPCODE)headPart.Read(PACKET_OPTION_UNIT.BYTE);
        }

        public RealmPacket(REALM_OPCODE pmOpcode)
        {
            headPart = new PacketPart();
            bodyPart = new PacketPart();
            headPart.Write(PACKET_OPTION_UNIT.UINT16, pmOpcode);
        }

        private REALM_OPCODE opcode;
        public REALM_OPCODE Opcode
        {
            get
            {
                return this.opcode;
